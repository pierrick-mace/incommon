...
int load_words(options *optvar) {
    ...
    if (optvar -> xfileptr != NULL) {
        IF_ERROR(parse_file(optvar -> xfileptr, optvar, file_excluded, optvar -> nb_files), 
                    F_ERROR)
    }
    ...
    for(size_t i = 0; i < optvar -> nb_files; i++) {
        if (i == 0) {
            ...
            IF_ERROR(parse_file(optvar -> fileptr[i], optvar, file_regular, i), F_ERROR)
        }
    }
...
int add_excluded_word(options *optvar, char *string) {
    ...
    IS_NULL_INST(hashtable_add(optvar -> hashtbl, s, counter), 
                    fprintf(stderr, "Erreur lors de l'ajout d'un element dans la table de hachage.\n"), 
                    F_ERROR)
